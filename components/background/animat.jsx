...
const BackgroundAnimation = () => {
    useEffect(() => {
        ...
        const adjustSphereSize = () => {
            ...
            const sphereSize = Math.min(windowWidth * 0.9, 900); // Adjust the maximum size as needed
            document.documentElement.style.setProperty('--sphere-size', `${sphereSize}px`);
        };

        window.addEventListener('resize', adjustSphereSize);
        adjustSphereSize(); // Call initially

        return () => {
            window.removeEventListener('resize', adjustSphereSize);
        };
    }, []);
    return (
        <motion.div className="sphere-animation">
